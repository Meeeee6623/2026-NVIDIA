...
def get_hamming_stats(file_path, n, target_bitstring):
    ...
    if (
        ...
        or pd.isna(target_bitstring)
        or target_bitstring == "N/A"
    ):
        return None, None
...
def main():
    ...
    hamming_results = df.apply(
        lambda row: get_hamming_stats(
            row["warm_start_file"], int(row["N"]), row["best_bitstring"]
        ),
        axis=1,
    )
    df[["min_hamming", "avg_hamming"]] = pd.DataFrame(
        hamming_results.tolist(), index=df.index
    )
