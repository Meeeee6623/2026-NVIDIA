...
@cudaq.kernel
def kernel_default(
    n: int, indices: list[int], indices2: list[int], theta_list: list[float]
):
...
def kernel_dna(
    ...
    indices: list[int],
    indices2: list[int],
    theta_list_CD: list[float],
    lamb_list_AD: list[float],
    CD: list[bool],
    AD: list[bool],
):
    qubits = cudaq.qvector(n)
...
@cudaq.kernel
def kernel_tensor_heavy(
    n: int, list_2: list[int], list_4: list[int], theta_list: list[float]
):
    qubits = cudaq.qvector(n)
    h(qubits)
    for t_val in theta_list:
        for i in range(0, len(list_2), 2):
            ...
            rx(np.pi / 2, qubits[idx1])
            cx(qubits[idx1], qubits[idx2])
            rz(t_val, qubits[idx2])
            cx(qubits[idx1], qubits[idx2])
            rx(-np.pi / 2, qubits[idx1])
...
def get_interactions_scaled(n):
    ...
    for i in range(n - 1):
        list_2.extend([i, i + 1])
    for i in range(n - 3):
        list_4.extend([i, i + 1, i + 2, i + 3])
    return list_2, list_4


def export_ready_for_cuda(
    quantum_population, n, filename, gpu_pop_size=8192, gpu_max_n=512
):
    host_buffer = np.random.choice([-1, 1], size=(gpu_pop_size, gpu_max_n)).astype(
        np.int8
    )
...
def run_simulation(n, shots, pop_size, output_file, variant, steps, lam_method):
    ...
    if variant == "default":
        ...
        result = cudaq.sample(
            kernel_jenga, n, list_2, list_4, thetas_CD, shots_count=shots
        )
    elif variant == "dna":
        result = cudaq.sample(
            kernel_dna,
        ...
        result = cudaq.sample(
            kernel_tensor_heavy, n, list_2, list_4, thetas_CD, shots_count=shots
        )

    top_strings = [
        bs
        for bs, count in sorted(result.items(), key=lambda x: x[1], reverse=True)[
            :pop_size
        ]
    ]
    pop_list = [
        (2 * np.array([int(b) for b in s]) - 1).astype(np.int8) for s in top_strings
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--variant",
        type=str,
        choices=["jenga", "dna", "beyblade", "tensor_heavy"],
        default="dna",
    )
    parser.add_argument(
        "--lambda_method",
        type=str,
        choices=["linear", "sqrt", "cuberoot", "trig"],
        default="trig",
    )
