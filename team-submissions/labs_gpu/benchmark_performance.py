            ...
            for variant in VARIANTS:
                for method in LAMBDA_METHODS:
                    ...
                    warm_start_path = os.path.join(
                        WARM_START_DIR, f"warm_{param_id}.bin"
                    )
                    log_file_path = os.path.join(LOG_DIR, f"log_{param_id}.log")

                    print(
                        f"[*] Testing N={n} | Variant={variant} | Method={method} | Steps={STEPS}"
                    )
                    ...
                    res_os = subprocess.run(
                        one_shot_cmd, capture_output=True, text=True
                    )
                    ...
                    # --- Parsing Metrics ---
                    total_moves, equiv_evals, best_bitstring = parse_runopt_metrics(
                        res_ro.stdout
                    )
